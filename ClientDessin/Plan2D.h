...
public:
	Plan2D() : Origine(Point2D(0,0)), I(Vecteur2D(0,500)), J(Vecteur2D(500,0)) {}
	...
	void ajouterForme(const Forme2D* f) {
		if (f->xMAX() > J.y + Origine.y) { //sort du repere dans l'axe y
			setAxeJ(f->xMAX() + 10);//laisse une petit marge de 10 ( arbitraire )
		}
		if (f->yMAX() > I.y + Origine.x) { //sort du repere dans l'axe x
			setAxeI(f->yMAX() + 10);
		}
